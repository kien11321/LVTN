...
@section('content')
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 10px;">
        <form method="GET" action="{{ route('sinhvien.index') }}" class="search-bar" style="flex: 1; min-width: 300px;">
            <input type="text" class="search-input" name="search" placeholder="Tìm kiếm theo tên hoặc MSSV..." value="{{ $search ?? '' }}">
            <button type="submit" class="btn btn-success">Tìm kiếm</button>
        </form>
        <div style="display: flex; gap: 10px; align-items: center;">
            <form method="POST" action="{{ route('sinhvien.import.post') }}" enctype="multipart/form-data" style="display: flex; align-items: center; gap: 10px;">
                @csrf
                <input type="file" 
                       name="file" 
                       accept=".xlsx,.xls"
                ...
                <tr>
                    ...
                    <td>
                        <div class="action-buttons">
                            ...
                            <form method="POST" action="{{ route('sinhvien.destroy', $sv->id) }}" style="display: inline;" onsubmit="return confirm('Bạn có chắc muốn xóa sinh viên này?')">
                                @csrf
                                @method('DELETE')
